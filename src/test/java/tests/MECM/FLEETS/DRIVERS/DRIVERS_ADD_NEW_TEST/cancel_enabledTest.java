...
public class cancel_enabledTest extends TestInit
{
    @Test
    public void cancel_enabled() throws Exception
    {
        ExtentTest t1 = pNode.createNode("cancel_enabled ", "cancel_enabled");
        Login.init(t1).validLogin("MECM");
        SubFleetsPage.init(t1).mobility().fleets();
        fleetDriverPage.init(t1).clikOnDriver();
        Thread.sleep(1000);
        fleetDriverPage.init(t1).clcikOnAddDriver();
        boolean isIt =  fleetDriverPage.init(t1).isCancelEnabled();
        if (isIt) {
            t1.pass("is enabled");
